        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <select
              ...
              className="px-4 py-2 rounded-xl border border-slate-200 bg-slate-50 text-slate-700 text-sm focus:outline-none focus:border-vibrant-blue w-full sm:w-auto"
            >
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        >
          <Card className="overflow-hidden">
            <div className="divide-y divide-slate-200 max-h-[500px] sm:max-h-[600px] overflow-y-auto">
              {filteredInquiries.map((inquiry) => (
                <button
                  key={inquiry.id}
                  onClick={() => setSelectedInquiry(inquiry)}
                  className={`w-full text-left p-3 sm:p-4 hover:bg-slate-50/50 transition-colors ${
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-900 truncate">{inquiry.user.name}</p>
                        <p className="text-xs text-slate-500">{inquiry.createdAt}</p>
                      </div>
                    </div>
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${getStatusColor(inquiry.status)}`}>
              ...
              <div className="p-4 sm:p-6 border-b border-slate-200">
                <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <h2 className="text-lg font-semibold text-slate-900 truncate">{selectedInquiry.subject}</h2>
